...
if( om_wpb_activated() && get_option(OM_THEME_PREFIX . 'disable_wpb_addons') != 'true' ) {
	...
	vc_set_shortcodes_templates_dir(TEMPLATE_DIR . '/wpb-4.7/vc_templates/');
		
	require_once (TEMPLATE_DIR . '/wpb-4.7/functions/attribute-types.php');
	require_once (TEMPLATE_DIR . '/wpb-4.7/functions/options.php');
	require_once (TEMPLATE_DIR . '/wpb-4.7/functions/misc.php');
	require_once (TEMPLATE_DIR . '/wpb-4.7/functions/custom_shortcodes.php');
	require_once (TEMPLATE_DIR . '/wpb-4.7/functions/modifications.php');
	...
	function om_wpb_admin_scripts() {
		if ( in_array( get_post_type(), vc_editor_post_types() )) {
			wp_enqueue_style( 'om_wpb_admin_styles', TEMPLATE_DIR_URI . '/wpb-4.7/assets/css/wpb-admin.css', array(), OM_THEME_VERSION );
			//wp_enqueue_script( 'om_wpb_custom_views', TEMPLATE_DIR_URI . '/wpb-4.7/assets/js/custom-views.js', array('jquery'), OM_THEME_VERSION, true);
		}
	}
	add_action( 'admin_enqueue_scripts', 'om_wpb_admin_scripts' );
	
	function om_wpb_front_scripts() {
		wp_register_script( 'vc_pie', TEMPLATE_DIR_URI . '/wpb-4.7/assets/js/jquery.vc_chart.js', array( 'jquery', 'waypoints', 'progressCircle' ), OM_THEME_VERSION, true );

		wp_register_script( 'om_js_composer_front', TEMPLATE_DIR_URI . '/wpb-4.7/assets/js/addon_js_composer_front.js', array('jquery', 'omLibraries', 'waypoints'), OM_THEME_VERSION, true );
		
		wp_register_script( 'om_charts', TEMPLATE_DIR_URI . '/wpb-4.7/assets/js/chart.min.js', array('jquery', 'omLibraries', 'waypoints'), OM_THEME_VERSION, true );
